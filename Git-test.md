...
_git fetch_ - эта команда связывается с удаленными репозиториями и забирает из него все изменения, которых у вас пока нет и сохраняет их локально.



GitHub

_git pull_ - рабоотает с комбинациями команд _git fetch_ и _git merge_, т.е. Git вначале забирает изменения из указанного удаленного репозитория, а затем пытается слить их с текущей веткой.

_git push_ - испольлзуется для установления связи с удаленным репозиторием, вычисления локальных изменений отсутствующих в нем, и собственно их передачи в вышеупомянутый репозиторий. Для нее нужна аутентификация.
